...
METADATA_BASENAMES = (INFO_BASENAME, "icon.png", "fanart.jpg", "LICENSE.txt")
...
def get_metadata_basenames(addon_metadata):
    return [(basename, basename) for basename in METADATA_BASENAMES] + [
        ("changelog.txt", "changelog-{}.txt".format(addon_metadata.version))
    ]
...
def parse_metadata(metadata_file):
    ...
    try:
        ...
        raise RuntimeError("Cannot open add-on metadata: {}".format(metadata_file))
    root = tree.getroot()
    addon_metadata = AddonMetadata(root.get("id"), root.get("version"), root)
    # Validate the add-on ID.
    if addon_metadata.id is None or re.search(r"[^a-z0-9._-]", addon_metadata.id):
...
def generate_checksum(archive_path, is_binary=True, checksum_path_opt=None):
    checksum_path = (
        "{}.md5".format(archive_path)
        if checksum_path_opt is None
        else checksum_path_opt
    )
...
def copy_metadata_files(source_folder, addon_target_folder, addon_metadata):
    for source_basename, target_basename in get_metadata_basenames(addon_metadata):
        ...
        if os.path.isfile(source_path):
            shutil.copyfile(
                source_path, os.path.join(addon_target_folder, target_basename)
            )


def on_remove_error(function, path, excinfo):
    exc_info_value = excinfo[1]
    if hasattr(exc_info_value, "errno") and exc_info_value.errno == errno.EACCES:
...
def fetch_addon_from_git(addon_location, target_folder):
    ...
    match = re.match(
        r"((?:[A-Za-z0-9+.-]+://)?.*?)(?:#([^#]*?))?(?::([^:]*))?$", addon_location
    )
    (clone_repo, clone_branch, clone_path_option) = match.groups()
    clone_path = (
        get_posix_path(os.path.join(".", ""))
        if clone_path_option is None
        else clone_path_option
    )
    ...
    try:
        ...
        copy_metadata_files(clone_source_folder, addon_target_folder, addon_metadata)

        return addon_metadata
    finally:
        shutil.rmtree(clone_folder, ignore_errors=False, onerror=on_remove_error)
    ...
    with zipfile.ZipFile(
        ...
        for root, _, files in os.walk(addon_location):
            ...
            for relative_path in files:
                source_path = os.path.realpath(os.path.join(root, relative_path))
                if source_path == archive_path:
                    # Don't include the archive within itself.
                    continue
                archive.write(source_path, os.path.join(relative_root, relative_path))
    generate_checksum(archive_path)
...
def fetch_addon_from_zip(raw_addon_location, target_folder):
    ...
    with zipfile.ZipFile(
        ...
        roots = frozenset(
            next(iter(path.split("/")), "") for path in archive.namelist()
        )
        if len(roots) != 1:
    ...
    if (
        not samefile(addon_source_folder, addon_target_folder)
        or os.path.basename(addon_location) != archive_basename
    ):
...
def get_addon_worker(addon_location, target_folder):
    ...
    thread = threading.Thread(
        target=lambda: fetch_addon_to_result_slot(
...
def create_repository(
    addon_locations, data_path, info_path, checksum_path, is_compressed, no_parallel
):
    # Import git lazily.
...
def main():
    ...
    parser.add_argument(
        "--checksum", "-c", help="Path for the addons.xml.md5 file [INFO.md5]"
    )
    parser.add_argument(
        "--compressed", "-z", action="store_true", help="Compress addons.xml with gzip"
    )
    parser.add_argument(
        "--no-parallel",
        "-n",
    ...
    checksum_path = (
        os.path.expanduser(args.checksum)
